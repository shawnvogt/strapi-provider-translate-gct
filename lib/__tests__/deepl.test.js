...
describe('gct provider', () => {
  ...
  beforeAll(() => {
    ...
    const translateHandler = async (req, res, ctx) => {
      ...
      if (stringByteLength(body || '') > GCT_API_MAX_REQUEST_SIZE) {
        ...
        describe('succeeds', () => {
          ...
          it('with a field larger than request size limit', async () => {
            ...
            const params = {
              sourceLocale: 'en',
              targetLocale: 'de',
              text: [faker.lorem.paragraphs(GCT_API_MAX_REQUEST_SIZE / 200)],
            }
            // when
            const result = await deeplProvider.translate(params)
                ...
                () =>
                  faker.lorem.paragraphs(
                    faker.datatype.number({
                      min: GCT_API_MAX_REQUEST_SIZE / 2000,
                      max: GCT_API_MAX_REQUEST_SIZE / 200,
                    })
                  ),
                textLength
              ),
